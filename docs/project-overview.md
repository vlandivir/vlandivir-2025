...
- **Frontends:** vanilla JS/HTML/CSS apps in `web/` (no build step), plus `telegram-app/` — React + Vite (GTD web app and Telegram Mini App)
...
| [reels-api.controller.ts](../src/reels-api.controller.ts) | `/reels-api` | Instagram reels archive: create/retry/delete, transcribe (Whisper), vision (frame extraction + LLM), tag/title generation, semantic search (`GET /search`), Q&A (`GET /ask`) + embeddings backfill (`POST /embed-all`); montage projects (`/projects…`: clips, order, ffmpeg `-c copy` trim, ZIP export for CapCut). Auth: Google session or `x-reels-api-key` (`EditAccessGuard`) |
| [reels-pages.controller.ts](../src/reels-pages.controller.ts) | `/reels`, `/reels/:id` | Owner-only reels notebook (Google session); per-reel OG tags |
...
- [reel-projects.service.ts](../src/services/reel-projects.service.ts) — named montage projects (`ReelProject` / `ReelProjectClip`): order clips, stream-copy trim to Spaces (`reels/projects/…`), ZIP export (`01-shortcode.mp4`…) for CapCut import
...
| `reels/` | Reels notebook + montage projects | Vanilla JS behind Google sign-in; player + side panel; projects panel for CapCut ZIP export |
