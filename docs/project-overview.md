...
| `trip/` | Shared trip photo/video album | Secret-link SPA; create at `/trip` (with “my albums” list from IndexedDB visits + owned via `contributorId`), album at `/trip/<secret>`; bilingual via single-file i18n; Google admin montage panel creates projects and picks clips (trim/export → desktop Trip Montage) |
