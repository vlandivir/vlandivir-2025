...
- [trip-projects.service.ts](../src/services/trip-projects.service.ts) — per-trip CapCut montage (`TripProject` / `TripProjectClip`): order videos, stream-copy trim to Spaces (`trips/{tripId}/projects/…`), background ZIP export (clips streamed to disk → Spaces `…/export.zip`, status in `exportZipStatus`/`exportZipProgress`); API gated to Google admin
...
| `trip/` | Shared trip photo/video album | Secret-link SPA; create at `/trip` (with “my albums” list from IndexedDB visits + owned via `contributorId`), album at `/trip/<secret>`; bilingual via single-file i18n; Google admin gets a CapCut montage panel |
