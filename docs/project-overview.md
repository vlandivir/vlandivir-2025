...
- [trip-projects.service.ts](../src/services/trip-projects.service.ts) — per-trip CapCut montage (`TripProject` / `TripProjectClip`): order videos, stream-copy trim to Spaces (`trips/{tripId}/projects/…`), ZIP export (`01-filename.mp4`…); API gated to Google admin
