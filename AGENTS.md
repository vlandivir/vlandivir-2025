...
- **Mini-app dev server:** `npm run web:mini-app:dev` serves the React/Vite app at `http://localhost:5173/mini-app/`, but it fetches `/mini-app-api/user`, so real profile data requires the backend (and thus secrets); without it the UI shows an error state.
