(() => {
  ...
  const ACTIVE_PROJECT_KEY = 'reels-active-project-id';
  ...
  const state = {
    ...
    pollTimer: null,
    projectsOpen: false,
    projects: [], // [{id, name, clipCount, …}]
    activeProject: null, // full project with clips, or null
    selectedClipId: null,
  };
  ...
  function mutationHeaders(extra = {}) {
    const headers = { ...extra };
    const key = getApiKey();
    if (key) headers['x-reels-api-key'] = key;
    return headers;
  }
  ...
  function renderDetails() {
    ...
    if (reel.status === 'ready' && reel.videoUrl) {
      const inActive = clipCountInActiveProject(reel.id);
      const addBtn = document.createElement('button');
      addBtn.className = 'mini-btn';
      addBtn.textContent = inActive
        ? `➕ В проект (${inActive})`
        : '➕ В проект';
      addBtn.addEventListener('click', () => addReelToProject(reel));
  ...
  function clipCountInActiveProject(reelId) {
    if (!state.activeProject?.clips) return 0;
    return state.activeProject.clips.filter((c) => c.reelId === reelId).length;
  }

  function setProjectsOpen(open) {
    state.projectsOpen = open;
    el('projects-toggle').classList.toggle('active', open);
    el('projects-panel').classList.toggle('hidden', !open);
    el('recent-section').classList.toggle('hidden', open);
    if (open) {
      void loadProjects();
  ...
  el('projects-toggle').addEventListener('click', () => {
    setProjectsOpen(!state.projectsOpen);
  });
  el('projects-close').addEventListener('click', () => setProjectsOpen(false));
  ...
  async function loadProjects() {
    ...
    const savedId = Number(localStorage.getItem(ACTIVE_PROJECT_KEY) || 0);
    const preferred =
      (state.activeProject &&
        state.projects.find((p) => p.id === state.activeProject.id)?.id) ||
      (savedId && state.projects.find((p) => p.id === savedId)?.id) ||
      state.projects[0]?.id;
    if (preferred) {
      await openProject(preferred);
    } else {
  ...
  function renderProjectsList() {
    ...
    state.projects.forEach((project) => {
      ...
      row.className =
        'project-row' +
        (state.activeProject?.id === project.id ? ' selected' : '');
      const name = document.createElement('span');
      name.className = 'project-row-name';
      name.textContent = project.name;
      const meta = document.createElement('span');
      meta.className = 'project-row-meta';
      meta.textContent = `${project.clipCount} клип.`;
  ...
  async function openProject(projectId) {
    ...
    state.activeProject = await response.json();
    localStorage.setItem(ACTIVE_PROJECT_KEY, String(projectId));
    if (
      state.selectedClipId &&
      !state.activeProject.clips.some((c) => c.id === state.selectedClipId)
    ) {
      state.selectedClipId = null;
    }
    renderProjectsList();
  ...
  function renderProjectDetail() {
    ...
    renameBtn.addEventListener('click', () => renameActiveProject());
    actions.appendChild(renameBtn);

    const exportBtn = document.createElement('button');
    exportBtn.type = 'button';
    exportBtn.className = 'primary-btn';
    exportBtn.textContent = 'Скачать ZIP';
    exportBtn.disabled = !project.clips.length;
    exportBtn.addEventListener('click', () => exportActiveProject(exportBtn));
    ...
    deleteBtn.addEventListener('click', () => deleteActiveProject());
    ...
    if (!project.clips.length) {
      ...
      empty.className = 'projects-empty';
      empty.textContent =
        'Клипов пока нет — откройте ролик и нажмите «В проект»';
      detail.appendChild(empty);
      return;
    }

    const clips = document.createElement('div');
    clips.className = 'project-clips';
    project.clips.forEach((clip, index) => {
      clips.appendChild(buildClipRow(clip, index));
    });
    detail.appendChild(clips);

    const selectedClip = project.clips.find(
      (c) => c.id === state.selectedClipId,
    );
    if (selectedClip) {
      detail.appendChild(buildTrimPanel(selectedClip));
    }
  }
  ...
  function buildClipRow(clip, index) {
    ...
    row.className =
      'project-clip' +
      (state.selectedClipId === clip.id ? ' selected' : '');

    const cover = document.createElement('div');
    cover.className = 'project-clip-cover';
    if (reel.coverUrl) {
      const img = document.createElement('img');
    ...
    body.type = 'button';
    body.className = 'project-clip-body';
    const title = document.createElement('div');
    title.className = 'project-clip-title';
    title.textContent = `${String(index + 1).padStart(2, '0')}. ${
      reel.title || reel.shortcode || `#${clip.reelId}`
    }`;
    const meta = document.createElement('div');
    meta.className = 'project-clip-meta';
    const range = formatTrimRange(clip, reel.duration);
    meta.textContent = [
      range,
      clip.trimmedVideoUrl ? 'обрезан' : null,
    ]
      .filter(Boolean)
      .join(' · ');
    body.appendChild(title);
    body.appendChild(meta);
    body.addEventListener('click', () => selectProjectClip(clip));
    row.appendChild(body);

    const tools = document.createElement('div');
    tools.className = 'project-clip-tools';
    ...
    down.title = 'Ниже';
    down.textContent = '↓';
    down.disabled = index >= state.activeProject.clips.length - 1;
    ...
    remove.title = 'Убрать';
    remove.textContent = '✕';
    remove.addEventListener('click', () => removeClip(clip));
  ...
  function formatTrimRange(clip, duration) {
  ...
  function buildTrimPanel(clip) {
    const reel = clip.reel || {};
    const duration =
      typeof reel.duration === 'number' && Number.isFinite(reel.duration)
        ? reel.duration
        : 0;

    const panel = document.createElement('div');
    panel.className = 'project-trim';

    const label = document.createElement('div');
    label.className = 'project-trim-label';
    label.textContent = 'Обрезка (без перекодирования)';
    panel.appendChild(label);

    const row = document.createElement('div');
    row.className = 'project-trim-row';

    const startInput = document.createElement('input');
    startInput.type = 'number';
    startInput.min = '0';
    startInput.step = '0.1';
    startInput.placeholder = 'Старт, с';
    startInput.value =
      clip.trimStartSec != null ? String(clip.trimStartSec) : '0';
    startInput.addEventListener('change', () => {
      const t = Number(startInput.value);
      if (Number.isFinite(t) && player.src) {
    ...
    endInput.min = '0';
    endInput.step = '0.1';
    endInput.placeholder = 'Конец, с';
    endInput.value =
      clip.trimEndSec != null
        ? String(clip.trimEndSec)
        : duration
          ? String(Math.round(duration * 10) / 10)
          : '';
    ...
    if (duration) {
      ...
      hint.className = 'project-trim-hint';
      hint.textContent = `Длительность ролика: ${formatDuration(duration)}`;
      panel.appendChild(hint);
    }

    const actions = document.createElement('div');
    actions.className = 'project-trim-actions';

    const markStart = document.createElement('button');
    markStart.type = 'button';
    markStart.className = 'mini-btn';
    markStart.textContent = 'Старт = сейчас';
    markStart.addEventListener('click', () => {
      if (!player.src) return;
      startInput.value = String(Math.round(player.currentTime * 10) / 10);
    });
    actions.appendChild(markStart);

    const markEnd = document.createElement('button');
    markEnd.type = 'button';
    markEnd.className = 'mini-btn';
    markEnd.textContent = 'Конец = сейчас';
    markEnd.addEventListener('click', () => {
      if (!player.src) return;
      endInput.value = String(Math.round(player.currentTime * 10) / 10);
    });
    actions.appendChild(markEnd);

    const saveBtn = document.createElement('button');
    saveBtn.type = 'button';
    saveBtn.className = 'mini-btn';
    saveBtn.textContent = 'Сохранить границы';
    saveBtn.addEventListener('click', async () => {
      const trimStartSec = Number(startInput.value);
      const trimEndSec = Number(endInput.value);
      if (!Number.isFinite(trimStartSec) || !Number.isFinite(trimEndSec)) {
        alert('Укажите старт и конец в секундах');
        return;
      }
      await patchClipTrim(clip.id, trimStartSec, trimEndSec);
    });
    ...
    applyBtn.textContent = clip.trimmedVideoUrl ? 'Перерезать' : 'Обрезать';
    applyBtn.addEventListener('click', async () => {
      ...
      applyBtn.textContent = 'Режу…';
      try {
        await patchClipTrim(clip.id, trimStartSec, trimEndSec);
        const response = await fetch(
          `${API_BASE}/projects/${state.activeProject.id}/clips/${clip.id}/trim`,
          { method: 'POST', headers: mutationHeaders() },
        );
        if (!response.ok) {
          const error = await response.json().catch(() => null);
          alert(error?.message || 'Не удалось обрезать');
          return;
        }
  ...
  function selectProjectClip(clip) {
    state.selectedClipId = clip.id;
    const reel = state.reels.find((r) => r.id === clip.reelId) || clip.reel;
    if (reel) {
      selectReel(reel, { fromUrl: true });
      const start = clip.trimStartSec ?? 0;
      const seek = () => {
      ...
      else player.addEventListener('loadedmetadata', seek);
    }
    renderProjectDetail();
  }

  async function patchClipTrim(clipId, trimStartSec, trimEndSec) {
    const response = await fetch(
      `${API_BASE}/projects/${state.activeProject.id}/clips/${clipId}`,
      {
        method: 'PATCH',
    ...
    if (!response.ok) {
      const error = await response.json().catch(() => null);
      alert(error?.message || 'Не удалось сохранить границы');
      throw new Error('trim patch failed');
    }
    await openProject(state.activeProject.id);
  ...
  async function moveClip(clipId, delta) {
    const ids = state.activeProject.clips.map((c) => c.id);
    const index = ids.indexOf(clipId);
    const next = index + delta;
    if (index < 0 || next < 0 || next >= ids.length) return;
    [ids[index], ids[next]] = [ids[next], ids[index]];
    const response = await fetch(
      `${API_BASE}/projects/${state.activeProject.id}/clips/order`,
      {
        method: 'PUT',
        headers: mutationHeaders({ 'Content-Type': 'application/json' }),
  ...
  async function removeClip(clip) {
    const title = clip.reel?.title || clip.reel?.shortcode || `#${clip.reelId}`;
    if (!confirm(`Убрать «${title}» из проекта?`)) return;
    const response = await fetch(
      `${API_BASE}/projects/${state.activeProject.id}/clips/${clip.id}`,
      { method: 'DELETE', headers: mutationHeaders() },
    );
    if (!response.ok) {
      alert('Не удалось убрать клип');
      return;
    }
  ...
  async function addReelToProject(reel) {
    ...
    if (!projectId) {
      if (!state.projects.length) {
        const name = prompt('Название нового проекта', 'Новый ролик');
        if (!name) return;
    ...
    const response = await fetch(`${API_BASE}/projects/${projectId}/clips`, {
      method: 'POST',
      headers: mutationHeaders({ 'Content-Type': 'application/json' }),
    ...
    if (!response.ok) {
      const error = await response.json().catch(() => null);
      alert(error?.message || 'Не удалось добавить в проект');
      return;
    }
    setProjectsOpen(true);
  ...
  el('project-create-form').addEventListener('submit', async (event) => {
    event.preventDefault();
    if (!(await ensureKey())) return;
    const input = el('project-name-input');
    const name = input.value.trim();
    if (!name) return;
    const submit = el('project-create-submit');
    submit.disabled = true;
    try {
      const created = await createProject(name);
      if (!created) return;
  ...
  async function renameActiveProject() {
    ...
    const name = prompt('Новое имя проекта', state.activeProject.name);
    if (!name || name.trim() === state.activeProject.name) return;
    const response = await fetch(
      `${API_BASE}/projects/${state.activeProject.id}`,
      {
        method: 'PATCH',
        headers: mutationHeaders({ 'Content-Type': 'application/json' }),
        body: JSON.stringify({ name }),
  ...
  async function deleteActiveProject() {
    ...
    if (
      !confirm(
        `Удалить проект «${state.activeProject.name}» и все его клипы?`,
      )
    ) {
      return;
    }
  ...
  async function exportActiveProject(button) {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/projects/${state.activeProject.id}/export.zip`,
        { headers: mutationHeaders() },
      );
      if (!response.ok) {
        const error = await response.json().catch(() => null);
        alert(error?.message || 'Не удалось скачать ZIP');
        return;
      }
      ...
      a.download = match?.[1] || `${state.activeProject.name}.zip`;
  ...
  // getApiKey is used above; declare helper early via function declaration hoist
  loadReels();
})();
