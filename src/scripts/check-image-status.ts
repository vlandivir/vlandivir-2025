...
async function checkImageStatus() {
    ...
    try {
        ...
        const percentageWithDescriptions = totalImages > 0 
            ? Math.round((imagesWithDescriptions / totalImages) * 100) 
            : 0;
        console.log(`📈 Percentage with descriptions: ${percentageWithDescriptions}%`);
        ...
        if (recentImagesWithDescriptions.length > 0) {
            ...
            recentImagesWithDescriptions.forEach(image => {
                console.log(`  - ID: ${image.id}, Date: ${image.createdAt.toISOString().split('T')[0]}`);
        ...
        if (imagesWithoutDescriptions > 0) {
            ...
            console.log(`  - Total estimated cost: $${(imagesWithoutDescriptions * 0.015).toFixed(2)}`);
        }
