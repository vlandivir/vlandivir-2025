...
async function testCollageWithCustomImages() {
    ...
    const customImagesDir = process.argv[2] || path.join(__dirname, '../../custom-images');
    ...
    try {
        ...
        if (imageFiles.length < 2) {
            ...
            console.log(`📁 Found ${imageFiles.length} images in: ${customImagesDir}`);
            return;
        }
...
async function createCollage(imageBuffers: Buffer[]): Promise<Buffer> {
    try {
        ...
        for (let i = 1; i < imageBuffers.length; i++) {
            ...
            console.log(`📐 Image ${i}: ${additionalImageWidth}x${additionalImageHeight} (original: ${imageMetadata.width}x${imageMetadata.height})`);
        }
